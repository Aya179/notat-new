...
namespace NotataSub.Controllers
{
    public class StudentMobileController : Controller
    {
       ...
       int facId, int departmentId, int StudyyearId)
        ...
        //https://localhost:44342/StudentMobile/getLectureFirstPages?lectureId=1
        public IActionResult getLectureFirstPages(int lectureId)
        {
           ...
           // string fileName = Path.Combine(_env.WebRootPath,Helper.lecturesPath + lecture.LectureId.ToString() +"/image-00001-01.jpg");
            string fileName = Path.Combine(_env.WebRootPath,Helper.lecturesPath1 + lecture.LectureId.ToString() +"/" + lecture.Linkurl + "_page-0001" + ".jpg");
        ...
        //https://localhost:44342/StudentMobile/getLecturePages?lectureId=1&pageId=_page-0006
        ...
        public IActionResult getLecturePage(int lectureId, string pageId)
        {
            byte[] buff = null;
            Lecture lecture = context.Lectures.Where(p => p.LectureId == lectureId).First();
            //  string fileName = Path.Combine(_env.WebRootPath, Helper.lecturesPath1 + lecture.LectureId.ToString() + "/" + pageId.ToString() + ".jpg");
            string fileName = Path.Combine(_env.WebRootPath, Helper.lecturesPath1 + lecture.LectureId.ToString() + "/" + lecture.Linkurl + pageId.ToString() + ".jpg");
            FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.Read);
            BinaryReader br = new BinaryReader(fs);
            long numBytes = new FileInfo(fileName).Length;
            buff = br.ReadBytes((int)numBytes);
        ...
        //https://localhost:44342/StudentMobile/getLecturePages?lectureId=1
        ...
        public IActionResult getLecturePages(int lectureId)
        {
            byte[] buff = null;
            Lecture lecture = context.Lectures.Where(p => p.LectureId == lectureId).First();
            string dirName = Path.Combine(_env.WebRootPath, Helper.lecturesPath1 + lecture.LectureId.ToString() );
