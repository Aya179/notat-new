...
namespace NotataSub.Controllers
{
    public class LecturesController : Controller
    {
        ...
        public async Task<IActionResult> Details(int? id)
        {
            ...
            var contents = provider.GetDirectoryContents(Path.Combine(Helper.lecturesPath1 , lecture.LectureId.ToString() ));
            var objFiles = contents.OrderBy(m => m.LastModified);

            List<string> ImageList = new List<string>();
            foreach (var item in objFiles.ToList())
            {
                ImageList.Add(Path.Combine("/" , Helper.lecturesPath1, Path.Combine(lecture.LectureId.ToString(), lecture.Linkurl , item.Name)));
            }
        ...
        public async Task<IActionResult> Create(LectureCreate lectureCreate)
        {
            if (ModelState.IsValid)
            {
                ...
                var uploadsFolderPath = Path.Combine(_env.WebRootPath, Helper.lecturesPath1);
                ...
                var uploadsFolderPath2 = Path.Combine(_env.WebRootPath, Helper.lecturesPath1 + lecture.LectureId);
                ...
                lecture.Linkurl = newlinkurl;

                ZipFile.ExtractToDirectory(filePath, Path.Combine(_env.WebRootPath, Helper.lecturesPath1 + lecture.LectureId));
